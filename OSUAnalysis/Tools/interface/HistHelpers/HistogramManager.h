...
enum value {
    Electron,
    Muon,
    NUMBER_OF_LEPTON_BINS
};

const boost::array<std::string, LeptonBin::NUMBER_OF_LEPTON_BINS> names = { {
        "elec",
        "mu" } };
}

namespace LepBinSummed {
enum value {
    lepton,
    NUMBER_OF_SUMMED_LEPTON_BINS
};

const boost::array<std::string, LepBinSummed::NUMBER_OF_SUMMED_LEPTON_BINS> names = { {
        "lepton" } };
}
...
public:
    ...
    void addH1D_LepBinned(std::string name, std::string title, unsigned int nBins, float xmin, float xmax);
    ...
    void setCurrentLepBin(unsigned int jetbin);
    ...
    boost::shared_ptr<TH1> H1D_LepBinned(std::string);
...
private:
    ...
    boost::multi_array<TH1CollectionRef, 2> lepBinned1DHists;
    std::vector<std::string> bJetBinned1DHistNames;
    std::vector<std::string> lepBinned1DHistNames;
    ...
    void createLepSummedHistograms(DataType::value);
};
