...
HistogramManager::HistogramManager() :
    ...
    lepBinned1DHists(boost::extents[DataType::NUMBER_OF_DATA_TYPES][LeptonBin::NUMBER_OF_LEPTON_BINS]),
    bJetBinned1DHistNames(),
    lepBinned1DHistNames(),
...
void HistogramManager::addH1D_LepBinned(std::string name, std::string title, unsigned int numberOfBins, float xmin,
        float xmax) {
    for (unsigned short lepbin = 0; lepbin < LeptonBin::NUMBER_OF_LEPTON_BINS; ++lepbin) {
        for (unsigned short type = DataType::DATA; type < DataType::NUMBER_OF_DATA_TYPES; ++type) {
            if (seenDataTypes.at(type)) {
                std::stringstream tmp_title;
                tmp_title << title << " (" << LeptonBin::names[lepbin] << ")";
                lepBinned1DHists[type][lepbin]->add(name, tmp_title.str(), numberOfBins, xmin, xmax);
            }
        }
    }
    lepBinned1DHistNames.push_back(name);
}
...
void HistogramManager::setCurrentLepBin(unsigned int lepbin) {
    if (lepbin > 2) {
        currentLepbin = 2;
    } else
        currentLepbin = lepbin;
}
...
boost::shared_ptr<TH1> HistogramManager::H1D_LepBinned(std::string histname) {
    return lepBinned1DHists[currentDataType][currentLepbin]->get(histname);
}
...
void HistogramManager::prepareForSeenDataTypes(const boost::array<bool, DataType::NUMBER_OF_DATA_TYPES>& seenDataTypes) {
    ...
    for (unsigned type = 0; type < DataType::NUMBER_OF_DATA_TYPES; ++type) {
        if (seenDataTypes.at(type)) {
            ...
            for (unsigned short lepbin = 0; lepbin < LeptonBin::NUMBER_OF_LEPTON_BINS; ++lepbin) {
                TH1CollectionRef coll(new TH1Collection());
                coll->setSuffix(LeptonBin::names.at(lepbin));
                lepBinned1DHists[type][lepbin] = coll;
            }
...
const std::string HistogramManager::assembleFilename(DataType::value type) const {
    ...
    if (Section == -1) {
      ...
      str << "_" << electronAlgo << "_" << jetAlgo << "_" << metAlgo << "_" << Section << ".root";
    }
    return str.str();
...
void HistogramManager::writeToDisk() {
    for (unsigned type = 0; type < DataType::NUMBER_OF_DATA_TYPES; ++type) {
        if (seenDataTypes.at(type)) {
            ...
            createLepSummedHistograms((DataType::value) type);
            ...
            for (unsigned short lepbin = 0; lepbin < LeptonBin::NUMBER_OF_LEPTON_BINS; ++lepbin) {
                lepBinned1DHists[type][lepbin]->writeToFile(histFiles.at(type));
            }
...
void HistogramManager::createLepSummedHistograms(DataType::value type){
    for (unsigned int nameIndex = 0; nameIndex < lepBinned1DHistNames.size(); ++nameIndex) {
        std::string name = lepBinned1DHistNames.at(nameIndex);
        for (unsigned short lepbinSum = 0; lepbinSum < LepBinSummed::NUMBER_OF_SUMMED_LEPTON_BINS; ++lepbinSum) {
            std::stringstream tmp_name, tmp_title;
            tmp_name << name << "_" << LepBinSummed::names[lepbinSum];
            boost::shared_ptr<TH1> hist((TH1*) lepBinned1DHists[type][lepbinSum]->get(name)->Clone(
                    tmp_name.str().c_str()));
            tmp_title << hist->GetTitle() << " (" << LepBinSummed::names[lepbinSum] << ")";
            hist->SetTitle(tmp_title.str().c_str());

            for (unsigned short lepbin = lepbinSum + 1; lepbin < LeptonBin::NUMBER_OF_LEPTON_BINS; ++lepbin) {
                hist->Add(lepBinned1DHists[type][lepbin]->get(name).get());
            }
            collection[type]->add(tmp_name.str(), hist);
        }
    }
