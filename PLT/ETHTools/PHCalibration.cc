...
void PHCalibration::LoadFitParameters(char *dirName, int phTrim, int chip)
{
	FILE *file;
...
double PHCalibration::GetFitPar(int chip, int col, int row, int par){ // par =0,1, ...
       if (par >= nFitParams){
         cout << "Parameter does not exist\n";
         return -1E6;
       }else{
         return fitParameter[par][chip][col][row];
       }
}
