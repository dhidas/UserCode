...
Double_t FitfcnF( Double_t *x, Double_t *par){
...
Double_t FitfcnTanF( Double_t *x, Double_t *par){
  return par[3] + par[2] * TMath::TanH(par[0]*x[0] - par[1]);
}


const char *FitfcnStr()
{
  return "TMath::Tan(par[0]*x[0] - par[4]) + par[1]*x[0]*x[0]*x[0] + par[5]*x[0]*x[0] + par[2]*x[0] + par[3]";
}
...
PHCalibrationFit::PHCalibrationFit(int aNumber){
  ...
  if (3 == fitMode) phFit = new TF1("phFit", FitfcnTanF, 50., 1500., 4);
  else phFit = new TF1("phFit", FitfcnF, -400., 1000., nFitParams);
...
void PHCalibrationFit::FitTanPol()
{
  ...
  if ( (upperPoint < n) && (lowerPoint < n) && ((x[upperPoint]-x[lowerPoint]) != 0) ) 
      ...
      for (int iCol = 0; iCol < 52; iCol++)
	{
	  for (int iRow = 0; iRow < 80; iRow++)
	      ...
	      if (n != 0)
		{
		  ...
		  for (int i = 0; i < nFitParams; i++){
	            fitParameters[chip][iCol][iRow][i]=phFit->GetParameter(i);
	          }
